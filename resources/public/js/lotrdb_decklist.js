...
function parse_lotrdb_decklist ( decklist ) {
  var deck = {};
  var crd;
...
$('#importdecklist').on('input',function() {
  $('#deckdata').val(parse_lotrdb_decklist($(this).val()))
});
...
$('#exportdeck').on('show.bs.modal',function (ev) { 
  ...
  $(this).find('input').val(parse_lotrdb_decklist($(ev.relatedTarget).data('export')));
});

$('#deletegroupmodal').on('show.bs.modal', function (ev) {
  var button = $(ev.relatedTarget);
  $(this).find('.modal-body').html('Are you sure you want to delete the Fellowship <b>' + button.data('name') + '</b>?');
  $('#deletemodalfname').val(button.data('name'));
  $('#deletemodalfuid').val(button.data('uid'));
});
