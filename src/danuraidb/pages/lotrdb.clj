  ...
  [:img.icon-sm.float-right {
    :src (str "/img/lotrdb/icons/"
              (if (some? (:sphere_name card)) 
                  ...
                  (str "pack-" (-> card :pack_name canonical)))
              ".png")}])
              ...
              [:div.card 
                ...
                [:div.card-body
                  ...
                  [:div.d-flex
                    ...
                    [:div.ml-auto [:i.fa.fa-solid.fa-code {:title (str card) :onclick (str "function e() {navigator.clipboard.writeText('" card "')};e()")}]]]
                    ]]]
            [:div.col-sm-6
              ...
              [:div.card ;.accordion-item
                [:div.card-header {:id (str "heading" (:cycle_position c))}
                    [:button.btn.btn-block.text-left {:type "button" :data-toggle "collapse" :data-target (str "#collapse" (:cycle_position c))} 
                      [:h5.mb-0
                        [:img.icon-sm.mr-3.me-3 {:src (str "\\img\\lotrdb\\icons\\" (icon_code c) ".png")}]
                        [:span (:name c)]]]]
                      ...
                      (for [s scens_in_cycle]
                        ...
                        (let [quests (->> cards (filter #(= (:type_code %) "quest")) (filter #(= (:encounter_name %) (:name s))))] 
                          ...
                          [:div.row ;.justify-content-between
                            [:span.h4 
                              [:img.icon-sm.mr-3.me-3 {:src (str "\\img\\lotrdb\\icons\\" (icon_code s) ".png")}]
                              [:span.mr-2 (:name s)]
                          ...
                          [:div.row
                            [:div.col-sm-6
                              [:h5 [:a {:href (str "/lotrdb/search?q=t:quest+n:" (:name s))} "Quests"]]
                              (for [q quests]
